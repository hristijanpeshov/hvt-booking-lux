...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
    ...
    @Query(value = "select extract(month from r.reservation_date) as month, count(ro.id) as num from Reservation as r " +
            "join Unit as u on u.id = r.unit_id " +
            "join RES_OBJECT as ro on ro.id = u.res_object_id where ro.creator_username=?1" +
            " and ?2 = extract(year from r.reservation_date)" +
            " group by month", nativeQuery = true)
    List<CreatorYearStatistics> findAnnualPropertyReservationCount(String creator, int year);

    @Query(value = "select ro.id, extract(month from r.reservation_date) as month, count(ro.id) as num from Reservation as r " +
            "join Unit as u on u.id = r.unit_id " +
            "join RES_OBJECT as ro on ro.id = u.res_object_id where ro.creator_username=?1" +
